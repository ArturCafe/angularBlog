...
import { FormsModule } from '@angular/forms';  // Import FormsModule here in the module (not in the component)
import { AuthuserService } from '../../../services/auth/authuser.service';

@Component({
  selector: 'app-create-video-posts',
  templateUrl: './create-video-posts.component.html',
  styleUrls: ['./create-video-posts.component.css'],
  imports: [AdminSideboardComponent, CommonModule, FormsModule],

})
export class CreateVideoPostsComponent implements OnInit {

  name = '';
  description = '';
  category = '';
  video: FileList | null = null;
  loading = false;
  categories: Category[] = [];
  ...
  createPost(): void {
    ...
    if (this.video && this.video.length > 0) {
      ...
      Array.from(this.video).forEach((file) => {
    ...
    this.postService.createVideoPost(formData).subscribe({
      next: () => {
