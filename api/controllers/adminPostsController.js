...
export const getPostsCategoryController = async (req, res) => {
...
export const deletePostsController = async (req, res) => {
  try {
    ...
    // Verificare validitate selectedIds
    if (!Array.isArray(selectedIds) || selectedIds.length === 0) {
      return res.status(400).json({
        success: false,
        message: "Invalid selectedIds. It must be a non-empty array."
      });
    }

    // Găsește posturile de șters
    const posts = await postModel.find({ _id: { $in: selectedIds } });

    if (posts.length === 0) {
      return res.status(404).json({
        success: false,
        message: "No posts found matching the provided IDs."
      });
    }
    ...
    // Așteaptă finalizarea ștergerii fișierelor
    await Promise.all([...photoDeletePromises, ...videoDeletePromises]);

    // Șterge posturile din baza de date
    const deleteResult = await postModel.deleteMany({ _id: { $in: selectedIds } });

    if (deleteResult.deletedCount > 0) {
      return res.status(200).json({
        success: true,
        message: "Posts and associated files successfully deleted."
      });
    }

    return res.status(404).json({
      success: false,
      message: "Posts not found in the database."
    });
    ...
    return res.status(500).json({
      success: false,
      message: "Failed to delete files or posts. " + error.message,  // Transmitere detalii eroare
      error: error.stack  // Adaugă detalii complete ale erorii pentru debugging
    });
  }
};
